...
export async function registerLiveMapRoutes(app: FastifyInstance) {
  ...
  app.get('/livemap/nearby', { preHandler: [authGuard] }, async (req: any) => {
    const { lat, lon, radius = 25 } = req.query as { 
      lat: number; 
      lon: number; 
      radius?: number; 
    };
  ...
  app.post('/livemap/location', { preHandler: [authGuard] }, async (req: any, reply) => {
    try {
  ...
  app.post('/livemap/races/:raceId/track', { preHandler: [authGuard] }, async (req: any, reply) => {
    try {
  ...
  app.post('/livemap/races/:raceId/telemetry', { preHandler: [authGuard] }, async (req: any, reply) => {
    try {
      const { raceId } = req.params as { raceId: string };
