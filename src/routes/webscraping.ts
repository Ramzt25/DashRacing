...
class BackendWebScrapingService {
  ...
  static async monitorPriceChanges(partIds: string[], alertThreshold: number = 10): Promise<any> {
    try {
      // Try external scraping API first
  ...
  static generateMockParts(query: string, filters: any) {
  ...
  private static generateMockMarketAnalysis(category: string, timeframe: string, priceRange?: any) {
  ...
  private static generateMockPriceMonitoring(partIds: string[], alertThreshold: number) {
    const monitoring: { [key: string]: any } = {};
  ...
  private static generateMockCompatibility(partId: string, carMake: string, carModel: string, carYear: number, engineType?: string) {
...
export async function registerWebScrapingRoutes(fastify: FastifyInstance) {
  ...
  fastify.post('/scraping/search-parts', {
    ...
    try {
      const { query, category, carMake, carModel, carYear, maxPrice, minPrice, sortBy } = request.body as {
        query: string;
        category?: string;
        carMake?: string;
        carModel?: string;
  ...
  fastify.post('/scraping/market-analysis', {
    ...
    try {
      const { category, timeframe = '30days', priceRange } = request.body as {
        category: string;
        timeframe?: string;
        priceRange?: { min: number; max: number };
      };
  ...
  fastify.post('/scraping/price-monitor', {
    ...
    try {
      const { partIds, alertThreshold = 10 } = request.body as {
        partIds: string[];
  ...
  fastify.post('/scraping/compatibility-check', {
    ...
    try {
      const { partId, carMake, carModel, carYear, engineType } = request.body as {
        partId: string;
        carMake: string;
        carModel: string;
        carYear: number;
      ...
      const result = await BackendWebScrapingService.checkCompatibility(
        partId,
        carMake,
        carModel,
        carYear,
  ...
  fastify.get('/scraping/trending-parts/:category?', async (request, reply) => {
    try {
      const category = (request.params as any).category || 'all';
