...
export async function registerMeetupRoutes(app: FastifyInstance) {
  // Get all public meetups
  app.get('/meetups', async (req) => {
    const { eventType, limit = 20 } = req.query as { eventType?: string; limit?: number };
  ...
  app.post('/meetups', { preHandler: [authGuard] }, async (req: any, reply) => {
    try {
  ...
  // Join a meetup
  app.post('/meetups/:id/join', { preHandler: [authGuard] }, async (req: any, reply) => {
    try {
      const { id: eventId } = req.params as { id: string };
      const { carId, notes } = req.body as { carId?: string; notes?: string };
  ...
  // Leave a meetup
  app.delete('/meetups/:id/leave', { preHandler: [authGuard] }, async (req: any, reply) => {
    try {
      const { id: eventId } = req.params as { id: string };
  ...
  app.patch('/meetups/:id/attendance', { preHandler: [authGuard] }, async (req: any, reply) => {
    try {
      const { id: eventId } = req.params as { id: string };
      const { status, carId, notes } = req.body as { 
  ...
  // Get my meetups
  app.get('/my/meetups', { preHandler: [authGuard] }, async (req: any) => {
    const { status } = req.query as { status?: 'upcoming' | 'past' | 'all' };
