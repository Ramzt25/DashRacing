...
export async function registerUserStatsRoutes(app: FastifyInstance) {
  ...
  app.get('/users/:userId/stats', async (req, reply) => {
    ...
    try {
      ...
      const bestLapTimes = user.raceResults
        ...
        .map(result => result.timeSeconds as number)
        .sort((a, b) => a - b)
        .slice(0, 5);
  ...
  app.get('/my/stats', { preHandler: [authGuard] }, async (req: any, reply) => {
    try {
      return app.inject({
        method: 'GET',
  ...
  app.get('/leaderboard', async (req, reply) => {
    try {
      const { limit = 10, raceType } = req.query as { limit?: number; raceType?: string };
  ...
  app.get('/cars/:carId/stats', async (req, reply) => {
    ...
    try {
      ...
      const topSpeeds = car.raceResults
        ...
        .map(result => result.topSpeed as number)
        .sort((a, b) => b - a)
        .slice(0, 5);
