...
export default async function mapsRoutes(fastify) {
    ...
    // Get nearby racers
    fastify.post('/nearby-racers', async (request, reply) => {
        try {
    ...
    // Get race routes
    fastify.post('/race-routes', async (request, reply) => {
        try {
            ...
            const routes = await GoogleMapsService.getRaceRoutes(startLocation.latitude, startLocation.longitude, endLocation.latitude, endLocation.longitude);
            reply.send({
                success: true,
                data: routes,
                raceId,
    ...
    fastify.post('/directions', async (request, reply) => {
        try {
    ...
    fastify.post('/road-conditions', async (request, reply) => {
        try {
            const { route } = request.body;
            if (!route || route.length === 0) {
                reply.status(400).send({
    ...
    fastify.post('/start-tracking', async (request, reply) => {
        try {
            ...
            const trackingSession = await GoogleMapsService.startLocationTracking(userId, raceId, trackingOptions);
            reply.send({
                success: true,
    ...
    fastify.post('/stop-tracking', async (request, reply) => {
        try {
