...
export async function registerAuthRoutes(app) {
    ...
    app.post('/auth/register', async (req, reply) => {
        ...
        if (exists)
            return reply.badRequest('Email or handle already in use');
    ...
    app.get('/auth/profile', { preHandler: [authGuard] }, async (req) => {
        const me = await prisma.user.findUnique({ where: { id: req.user.sub } });
        return {
            id: me.id,
            email: me.email,
    ...
    app.post('/auth/upgrade', { preHandler: [authGuard] }, async (req, reply) => {
        try {
    ...
    app.post('/auth/cancel-subscription', { preHandler: [authGuard] }, async (req, reply) => {
        try {
            const userId = req.user.sub;
            // In a real app, you would:
