...
export class EdmundsApiService {
    static API_KEY = process.env.EDMUNDS_API_KEY || '';
    static BASE_URL = 'https://api.edmunds.com/api/vehicle/v2';
    /**
     ...
     */
    static async getVehicleDetails(make, model, year) {
        try {
            if (!this.API_KEY) {
                console.warn('Edmunds API key not configured, using mock data');
                return this.getMockVehicleDetails(make, model, year);
            }
            const response = await fetch(`${this.BASE_URL}/${make}/${model}/${year}?api_key=${this.API_KEY}`);
            if (!response.ok) {
     ...
     */
    static async getVehicleTrims(make, model, year) {
        try {
            if (!this.API_KEY) {
                console.warn('Edmunds API key not configured, using mock data');
                return this.getMockVehicleTrims(make, model, year);
            }
            const response = await fetch(`${this.BASE_URL}/${make}/${model}/${year}/styles?api_key=${this.API_KEY}`);
            if (!response.ok) {
                throw new Error(`Edmunds API error: ${response.statusText}`);
            }
            const data = await response.json();
     ...
     */
    static async getVehiclePricing(make, model, year, trim, zip) {
        try {
            if (!this.API_KEY) {
                console.warn('Edmunds API key not configured, using mock data');
            ...
            const response = await fetch(`${this.BASE_URL}/${make}/${model}/${year}/pricing?${queryParams.toString()}`);
            if (!response.ok) {
                if (response.status === 404)
                    return null;
                throw new Error(`Edmunds API error: ${response.statusText}`);
     ...
     */
    static async getVehicleSpecifications(make, model, year) {
        try {
            if (!this.API_KEY) {
                return this.getMockVehicleSpecifications(make, model, year);
            }
            const response = await fetch(`${this.BASE_URL}/${make}/${model}/${year}/specs?api_key=${this.API_KEY}`);
            if (!response.ok) {
                throw new Error(`Edmunds API error: ${response.statusText}`);
            }
     ...
     * Get vehicle reviews
     */
    static async getVehicleReviews(make, model, year) {
        try {
            if (!this.API_KEY) {
                return this.getMockVehicleReviews(make, model, year);
            }
            const response = await fetch(`${this.BASE_URL}/${make}/${model}/${year}/reviews?api_key=${this.API_KEY}`);
            if (!response.ok) {
                throw new Error(`Edmunds API error: ${response.statusText}`);
            }
            const data = await response.json();
     ...
     * Get vehicle recalls
     */
    static async getVehicleRecalls(make, model, year) {
        try {
            if (!this.API_KEY) {
                return this.getMockVehicleRecalls(make, model, year);
            }
            const response = await fetch(`${this.BASE_URL}/${make}/${model}/${year}/recalls?api_key=${this.API_KEY}`);
            if (!response.ok) {
                throw new Error(`Edmunds API error: ${response.statusText}`);
            }
            const data = await response.json();
     ...
     */
    static async compareVehicles(vehicles) {
        try {
            const vehicleDetails = await Promise.all(vehicles.map(v => this.getVehicleDetails(v.make, v.model, v.year)));
            return {
                vehicles: vehicleDetails.filter(Boolean),
                comparison: this.generateComparison(vehicleDetails.filter(Boolean)),
            };
        }
        catch (error) {
    ...
    static async getVehicleMakes() {
        try {
            ...
            const data = await response.json();
            return data.makes?.map((make) => make.name) || this.getMockVehicleMakes();
        }
        catch (error) {
    ...
    static async getModelsForMake(make) {
        try {
            ...
            const data = await response.json();
            return data.models?.map((model) => model.name) || this.getMockModelsForMake(make);
        }
        catch (error) {
    ...
    static getMockVehicleSearch(params) {
        const allVehicles = [
        ...
        return allVehicles.filter(v => {
            if (params.make && v.make.toLowerCase() !== params.make.toLowerCase())
                return false;
            if (params.model && v.model.toLowerCase() !== params.model.toLowerCase())
                return false;
            if (params.year && v.year !== params.year)
                return false;
            return true;
        }).slice(0, params.limit || 20);
    }
    static getMockVehicleDetails(make, model, year) {
        return {
            id: `${make}-${model}-${year}`,
            make,
            model,
    ...
    static getMockVehicleTrims(make, model, year) {
        return [
            {
    ...
    static getMockVehiclePricing(make, model, year, trim) {
        return {
    ...
    static getMockVehicleSpecifications(make, model, year) {
        return {
    ...
    static getMockVehicleReviews(make, model, year) {
        return [
            {
                id: '1',
                rating: 4.5,
    ...
    static getMockVehicleRecalls(make, model, year) {
        return []; // Most vehicles have no active recalls
    }
    static getMockPopularVehicles() {
        return [
    ...
    static getMockModelsForMake(make) {
        const makeModels = {
    ...
    static transformVehicleSearchResults(data) {
        // Transform Edmunds API response to our format
        return data.makes?.flatMap((make) => make.models?.flatMap((model) => model.years?.map((year) => ({
    ...
    static transformVehicleDetails(data) {
        // Transform Edmunds API response to our format
        return {
            id: data.id || 'unknown',
    ...
    static transformVehicleTrims(data) {
    ...
    static transformVehiclePricing(data, make, model, year, trim) {
        return {
            vehicleId: `${make}-${model}-${year}`,
            make,
            model,
    ...
    static transformPopularVehicles(data) {
        return data.popular?.map((vehicle) => ({
            id: vehicle.id,
            make: vehicle.make,
            model: vehicle.model,
            year: vehicle.year,
    ...
    static generateComparison(vehicles) {
        if (vehicles.length < 2)
            return {};
        return {
            performance: {
                mostPowerful: vehicles.reduce((prev, current) => prev.engine.horsepower > current.engine.horsepower ? prev : current),
                mostFuelEfficient: vehicles.reduce((prev, current) => prev.fuelEconomy.combined > current.fuelEconomy.combined ? prev : current),
            },
            pricing: {
                mostAffordable: vehicles.reduce((prev, current) => prev.msrp < current.msrp ? prev : current),
                bestValue: vehicles.reduce((prev, current) => prev.tmv < current.tmv ? prev : current),
            },
            specifications: {
                largest: vehicles.reduce((prev, current) => prev.dimensions.length > current.dimensions.length ? prev : current),
                safest: vehicles.reduce((prev, current) => prev.safety.overallRating > current.safety.overallRating ? prev : current),
            },
        };
    }
}
//# sourceMappingURL=EdmundsApiService.js.map